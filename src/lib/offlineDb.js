...
export async function initOfflineDb() {
  return openDB(DB_NAME, DB_VERSION, {
    upgrade(db, oldVersion) {
      ...
      if (!db.objectStoreNames.contains("sales_local")) db.createObjectStore("sales_local", { keyPath: "id" });
      ...
      if (!db.objectStoreNames.contains("branch_config")) db.createObjectStore("branch_config", { keyPath: "id" });
...
export async function getCashDayOffline({ branchId, date }) {
  ...
  const registers = (await db.getAllFromIndex("cash_registers", "branch_id", branchId)).filter(r => isMatch(r.created_at)).sort((a,b) => b.created_at.localeCompare(a.created_at));
  const expenses = (await db.getAllFromIndex("cash_expenses", "branch_id", branchId)).filter(e => isMatch(e.created_at)).sort((a,b) => b.created_at.localeCompare(a.created_at));
  const localSales = (await db.getAll("sales_local")).filter(s => s.branch_id === branchId && isMatch(s.created_at));
  const cachedSales = (await db.getAllFromIndex("sales_cache", "branch_id", branchId)).filter(s => isMatch(s.created_at));
  const allSales = [...localSales, ...cachedSales].filter(s => String(s.payment_method || "").toLowerCase().includes("efectivo")).sort((a,b) => b.created_at.localeCompare(a.created_at));
  const finalSales = [];
