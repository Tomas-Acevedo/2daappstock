...
import { Banknote, Upload, Trash2, Calendar, Loader2 } from 'lucide-react';
...
const ExpensesPage = () => {
  ...
  // Función para formatear la fecha correctamente en la tabla sin error de zona horaria
  const formatDisplayDate = (dateStr) => {
    if (!dateStr) return "-";
    // Extraemos solo la fecha (YYYY-MM-DD) y reemplazamos guiones por barras
    // Esto evita que JS lo convierta a UTC y le reste horas.
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="mb-4 space-y-1">
           ...
           <div className="flex items-center gap-3">
             <label className="cursor-pointer bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 px-3 py-2 rounded-md text-sm font-medium transition-colors w-full sm:w-auto text-center">
               Elegir archivos
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-1">
           <label className="text-xs font-semibold text-gray-500 uppercase">Moneda del historial</label>
      ...
      <div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading && expenses.length === 0 ? (
                   <tr><td colSpan="5" className="text-center py-10"><Loader2 className="w-6 h-6 animate-spin mx-auto text-gray-300" /></td></tr>
                ) : expenses.length === 0 ? (
                   <tr><td colSpan="5" className="text-center py-10 text-gray-400">No hay gastos en este período.</td></tr>
                ) : (
                  expenses.map(exp => (
                    <tr key={exp.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap">
                        {/* ✅ CORRECCIÓN AQUÍ: Usamos la función formatDisplayDate */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-green-50 border border-green-100 rounded-xl p-6 shadow-sm">
            ...
            <div className="text-3xl font-extrabold text-gray-900">{formatCurrency(totalARS, 'ARS')}</div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 shadow-sm">
            ...
            <div className="text-3xl font-extrabold text-gray-900">{formatCurrency(totalUSD, 'USD')}</div>
          </div>
        </div>
      </div>
    </motion.div>
