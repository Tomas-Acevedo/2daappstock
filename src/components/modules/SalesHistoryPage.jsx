...
import { Calendar, Search, Trash2, Edit, ChevronLeft, ChevronRight, Wallet, Loader2, FileText } from 'lucide-react';
...
const SalesHistoryPage = () => {
  ...
  const fetchUserRole = async () => {
    ...
    if (user) {
      ...
      if (profile) setUserRole(profile.role.trim().toLowerCase());
    }
  };

  const fetchBranchDetails = async () => {
    const { data } = await supabase.from('branches').select('name, logo_url, address, tel, allow_sales_edit_delete').eq('id', branchId).single();
    if (data) {
  ...
  const fetchPaymentMethods = async () => {
    const { data } = await supabase.from('payment_methods').select('name').eq('branch_id', branchId);
    ...
    element.innerHTML = `
      <div style="font-family: Arial, sans-serif; padding: 40px; color: #333; background: white; width: 750px; margin: 0 auto;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 40px;">
          ...
          <tbody>
            ${sale.sale_items.map(p => `
              <tr>
                <td style="padding: 15px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px;">${p.product_name}</td>
                <td style="padding: 15px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px; text-align: center;">${p.quantity}</td>
                <td style="padding: 15px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px; text-align: right;">$${Number(p.unit_price).toLocaleString('es-AR')}</td>
                <td style="padding: 15px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px; text-align: right; font-weight: bold;">$${(p.unit_price * p.quantity).toLocaleString('es-AR')}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  const fetchSales = useCallback(async () => {
    ...
    setLoading(true);
    try {
      let query = supabase.from('sales').select(`*, sale_items (product_id, quantity, product_name, unit_price)`).eq('branch_id', branchId).order('created_at', { ascending: false }).range(page * pageSize, (page + 1) * pageSize - 1);
      let totalQuery = supabase.from('sales').select('total, payment_method').eq('branch_id', branchId);

      if (isOwner) {
        if (dateRange.start) { query = query.gte('created_at', `${dateRange.start}T00:00:00-03:00`); totalQuery = totalQuery.gte('created_at', `${dateRange.start}T00:00:00-03:00`); }
        if (dateRange.end) { query = query.lte('created_at', `${dateRange.end}T23:59:59-03:00`); totalQuery = totalQuery.lte('created_at', `${dateRange.end}T23:59:59-03:00`); }
        if (paymentFilter !== 'all') { query = query.eq('payment_method', paymentFilter); totalQuery = totalQuery.eq('payment_method', paymentFilter); }
      } else {
      ...
      setSummaryData({ total: (totalsRes.data || []).reduce((acc, s) => acc + Number(s.total), 0) });
    } catch (error) { toast({ title: "Error cargando historial", variant: "destructive" }); } finally { setLoading(false); }
  }, [branchId, roleLoading, userRole, page, dateRange, paymentFilter, isOwner]);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div><h1 className="text-3xl font-black text-gray-900">Historial de Ventas</h1><p className="text-sm text-gray-500 font-medium">Sucursal: {branchDetails.name}</p></div>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? <tr><td colSpan="6" className="text-center p-10 text-lg text-gray-400 font-medium">Cargando ventas...</td></tr> : sales.length === 0 ? <tr><td colSpan="6" className="text-center p-12 text-gray-500 text-lg">No se encontraron ventas.</td></tr> : sales.map((sale) => (
                <tr key={sale.id} className="hover:bg-indigo-50/40 transition-colors">
                  ...
                  <td className="px-6 py-6 text-base font-bold text-gray-900 align-top">{sale.customer_name || "Cliente General"}</td>
                  <td className="px-6 py-6 align-top">
                    <div className="flex flex-col gap-2">{sale.sale_items?.map((item, idx) => (
                      <span key={idx} className="text-sm text-gray-800 font-bold max-w-[350px] flex items-start gap-2">
                        <span className="text-indigo-600 font-black min-w-[25px]">{item.quantity}x</span><span className="flex-1">{item.product_name}</span><span className="text-gray-400 font-semibold text-xs whitespace-nowrap">({formatCurrency(item.unit_price)})</span>
                      </span>
                    ))}</div>
                  </td>
